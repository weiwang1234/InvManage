...
public interface ProductProcessingDetailRepository extends JpaRepository<ProductProcessingDetail, Long> {
    ...
    @Query(value = "select  outputproductid ,outputproductname ,sum(outputcount)  from productprocessing_detail   where  " +
            " processingdetaildate  like CONCAT(:month, '%')  group by   outputproductid ,outputproductname ",
            nativeQuery = true)
    List<ProductProcessingDetail> getroductProcessingDetailSummary(@Param("month") String month);
